...
*** TODO analog interface design
- design the analog interface between the ir sensor and adc
*** TODO real time data acquisition
...
*** TODO design a low-pass filter
- use either excel or filterpro (probably filterpro will be easier,
  windows-only), to design a low-pass filter to filter the adc data in
  real time
*** TODO plot the data on the lcd
- plot either:
  - voltage vs. time
