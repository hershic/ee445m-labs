...
call =os_signal= from a periodic interrupt. place =os_wait= in the loop of
a main program and watch the loop execute after =os_signal= is called.
*** TODO profiling compile-time flag
- compiles minimally intrusive pin-toggling code to
